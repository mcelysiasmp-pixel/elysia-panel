...
export class FilesService {
  ...
  private async connFor(serverId: string, user: AuthenticatedUser) {
    ...
    return {
      server,
      params: { host: server.node.grpcHost, port: server.node.grpcPort },
    };
  }

  async list(serverId: string, path: string, user: AuthenticatedUser) {
    const { server, params } = await this.connFor(serverId, user);
    const res = await this.nodeClient.call<any, { entries: FileEntryDto[] }>(
      server.nodeId,
      params,
      'ListFiles',
      {
  ...
  async read(serverId: string, path: string, user: AuthenticatedUser) {
    const { server, params } = await this.connFor(serverId, user);
    const res = await this.nodeClient.call<any, { content: Buffer }>(
      server.nodeId,
      params,
      'ReadFile',
      {
  ...
  async write(
    ...
    const { server, params } = await this.connFor(serverId, user);
    await this.nodeClient.call(server.nodeId, params, 'WriteFile', {
      server_uuid: server.uuid,
      path,
      content,
    });
  ...
  async delete(serverId: string, path: string, user: AuthenticatedUser) {
    const { server, params } = await this.connFor(serverId, user);
    await this.nodeClient.call(server.nodeId, params, 'DeleteFile', {
      server_uuid: server.uuid,
      path,
    });
    await this.audit.log({
  ...
  async mkdir(serverId: string, path: string, user: AuthenticatedUser) {
    const { server, params } = await this.connFor(serverId, user);
    await this.nodeClient.call(server.nodeId, params, 'CreateDirectory', {
      server_uuid: server.uuid,
      path,
    });
    await this.audit.log({
